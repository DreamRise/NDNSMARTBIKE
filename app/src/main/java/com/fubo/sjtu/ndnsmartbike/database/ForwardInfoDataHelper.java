...
public class ForwardInfoDataHelper {
    ...
    private static ForwardInfoDataHelper mForwardInfoDatHelper;

    private ForwardInfoDataHelper(Context context) {
    ...
    public static ForwardInfoDataHelper getInstance(Context context) {
        if (mForwardInfoDatHelper == null) {
            mForwardInfoDatHelper = new ForwardInfoDataHelper(context);
        }
        return mForwardInfoDatHelper;
    }
    ...
    public List<ForwardInfo> selectForwardInfoAll() {
        Cursor cursor = mSQLiteDatabase.query(ForwardInfoSQLiteHelper.TB_NAME, null, null, null,
                null, null, null);
    ...
    public List<ForwardInfo> selectForwardInfoByInterstName(String interestName) {
        Cursor cursor = mSQLiteDatabase.query(ForwardInfoSQLiteHelper.TB_NAME, null, ForwardInfo
                .INTEREST_NAME + " =? and " + ForwardInfo.FLAG + " = " + ForwardInfo
                .FORWARD_FLAG_NORMAL + " and " + ForwardInfo.TYPE + " = " + ForwardInfo
                .FORWARD_INTEREST_TYPE, new String[]{interestName}, null, null, null);
        List<ForwardInfo> forwardInfos = new ArrayList<>();
        while (cursor.moveToNext()) {
            ForwardInfo forwardInfo = getForwardInfoFromCursor(cursor);
            forwardInfos.add(forwardInfo);
    ...
    private ForwardInfo getForwardInfoFromCursor(Cursor cursor) {
        ...
        if (cursor != null) {
            ...
            forwardInfo.setInterestName(cursor.getString(cursor.getColumnIndex(ForwardInfo
                    .INTEREST_NAME)));
            forwardInfo.setType(cursor.getInt(cursor.getColumnIndex(ForwardInfo.TYPE)));
            forwardInfo.setInterestFrom(cursor.getString(cursor.getColumnIndex(ForwardInfo
                    .INTEREST_FROM)));
        }
        return forwardInfo;
    }
    ...
    public Boolean hasForwardInfoWithSameInterestFrom(String interestFrom) {
        Cursor cursor = mSQLiteDatabase.query(ForwardInfoSQLiteHelper.TB_NAME, null, ForwardInfo
                .INTEREST_FROM + "=? and " + ForwardInfo.FLAG + "=" + ForwardInfo
                .FORWARD_FLAG_NORMAL + " and " + ForwardInfo.TYPE + "=" + ForwardInfo
                .FORWARD_INTEREST_TYPE, new String[]{interestFrom}, null, null, null);
    ...
    public ForwardInfo selectForwardInfoByDataName(String dataName) {
        Cursor cursor = mSQLiteDatabase.query(ForwardInfoSQLiteHelper.TB_NAME, null, ForwardInfo
                .DATA_NAME + "=? and " + ForwardInfo.FLAG + " = " + ForwardInfo
                .FORWARD_FLAG_NORMAL + " and " + ForwardInfo.TYPE + "=" + ForwardInfo
                .FORWARD_DATA_TYPE, new String[]{dataName}, null, null, null);
        if (!cursor.moveToFirst())
            return null;
        else {
            ForwardInfo forwardInfo = getForwardInfoFromCursor(cursor);
    ...
    public void insertForwardInfo(ForwardInfo forwardInfo) {
        ...
        values.put(ForwardInfo.BUILD_DATE, SimpleDateFormatUtil.formatUtilDate(forwardInfo
                .getBuildDate(), SimpleDateFormatUtil.NORMAL_UTIL_DATE_FORMAT));
