...
public class crc16_ccitt {
	private static final int[] TABLE = new int[] { 0x0000, 0x1021, 0x2042,
	...
	// 计算crc16
	public static int calc_crc16_ccitt(final byte[] data, int offset, int len) {
		int result = 0;
		for (int ii = 0; ii < len; ++ii) {
			result = (TABLE[(result >> 8 ^ data[ii + offset]) & 0xff]
					^ (result << 8)) & 0xFFFF;
		}
		return result;
	}
